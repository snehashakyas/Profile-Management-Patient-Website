...
const addPatient = asyncHandler(async (req, res) => {
  ...
  if (error) {
    logger.info(error);
    return res.status(403).json(error.details); // returns error array from JOI
  }
  ...
  res.status(201).json(patient); // sends back patient
});
...
const editPatient = asyncHandler(async (req, res) => {
  ...
  // user check
  if (!req.user) {
    // if user does not exist. check for user.
  ...
  const editedPatient = await Patient.findByIdAndUpdate(
    ...
    { new: true }
  ); // req.body is the body text stored in req, and new: true creates a new one if it doesnt exist yet
